...
async function classifyWithOcb(url, clientName) {
  const baseUrl = process.env.OCB_URL || 'http://localhost:3008';
  try {
    const startRes = await fetch(`${baseUrl}/classify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        urls: [url],
        clientName,
        email: process.env.OCB_REPORT_EMAIL || 'damian@mooreiq.ai',
      }),
    });
    if (!startRes.ok) throw new Error(`OCB classify failed: HTTP ${startRes.status}`);
    const { runId } = await startRes.json();
    if (!runId) throw new Error('OCB classify response missing runId');
    ...
    while (Date.now() < deadline) {
      ...
      if (['done', 'complete', 'completed'].includes(status) || run.done === true) {
        const candidates = run.results || run.urls || run.pages || run.data || [];
        const result = Array.isArray(candidates)
          ? candidates.find(entry => entry.url === url) || candidates[0]
          : candidates[url] || candidates.result || candidates;
...
async function startOptimizeBatch(batchId, items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const ocb = await classifyWithOcb(item.url, item.clientName);
    const seedRow = {
