  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      ...
      const result = await runPipeline({
        ...payload,
        optimization: { url: item.url, guidance: item.guidance, beforeText, recommendations },
      });
      if (result.supabaseError) {
        ...
        await getSupabase().from(ARTICLE_TABLE).update({
          change_report: { bullets: report.bullets, ocb, recommendations, generated_at: generatedAt },
        }).eq('article_id', articleId);
        await getSupabase().from(BATCH_TABLE).update({
