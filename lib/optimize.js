...
function buildPreservationRequirements(html, parsed = extractPageSections(html)) {
  const links = [];
  const markdown = [parsed.intro?.text, ...(parsed.sections || []).map(section => section.text)].filter(Boolean).join('\n');
  for (const match of markdown.matchAll(/\[([^\]]+)\]\(([^)]+)\)/g)) {
    links.push({ anchor: match[1], href: match[2] });
  }
  return {
    headings: [parsed.h1, ...(parsed.sections || []).map(section => section.heading)].filter(Boolean),
    links,
  };
}

function preservationIssues(html, requirements = {}) {
  const headings = new Set([...String(html || '').matchAll(/<h[12]\b[^>]*>([\s\S]*?)<\/h[12]>/gi)]
    .map(match => normalizePreservedText(match[1])));
  const links = [...String(html || '').matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)]
    .map(match => ({ href: normalizePreservedUrl(match[1]), anchor: normalizePreservedText(match[2]) }));
  const issues = [];
  for (const heading of requirements.headings || []) {
    if (!headings.has(normalizePreservedText(heading))) issues.push(`Missing original heading: ${heading}`);
  }
  for (const link of requirements.links || []) {
    const found = links.some(candidate => candidate.href === normalizePreservedUrl(link.href)
      && candidate.anchor === normalizePreservedText(link.anchor));
    if (!found) issues.push(`Missing original link: ${link.anchor} (${link.href})`);
  }
  return issues;
}
