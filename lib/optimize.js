...
function missingSectionSource(originalText, output) {
  ...
  for (const heading of want.headings) {
    if (!consumeExact(got.headings, heading, value => value)) missing.push(`heading "${heading}"`);
  }
  for (const link of want.links) {
    if (!consumeExact(got.links, link, value => value)) missing.push(`link ${link}`);
  }
  return missing;
}
...
async function startOptimizeBatch(batchId, items) {
  ...
  await runPool(items.length, async (i) => {
    ...
    const ocb = await ocbLane(() => classifyWithOcb(item.url, item.clientName, item.keyword));
