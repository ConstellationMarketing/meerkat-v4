 ...
 *    once the article has been quiet for DEBOUNCE_MS (30s). New articles
 ...
 *
 * Jobs run one article at a time (its languages in parallel). User-triggered
 * jobs (edit / new-article) take priority over background reconcile jobs so
 * editors see updated translations quickly even during a backfill.
 */

const { runTranslation, SUPPORTED_LANGUAGES } = require('./translate');
const { listArticlesTranslationState } = require('./supabase');

const DEBOUNCE_MS = 30 * 1000;            // fire 30s after last edit
...
// Job queue with two priorities: user-triggered jobs (edits, new articles)
// jump ahead of background reconcile jobs so an editor never waits behind a
// backfill sweep. One article at a time; its languages run in parallel.
const jobs = [];
let working = false;
...
function enqueueRun(articleId, langs = SUPPORTED_LANGUAGES, reason = 'queue', { priority = false } = {}) {
  const todo = langs.filter((lang) => !inFlight.has(`${articleId}:${lang}`));
  if (todo.length === 0) return;

  for (const lang of todo) inFlight.add(`${articleId}:${lang}`);

  const job = { articleId, langs: todo, reason };
...
async function pump() {
  ...
  try {
    while (jobs.length > 0) {
      ...
      await Promise.all(
        job.langs.map(async (lang) => {
          try {
            await runTranslation(job.articleId, lang);
            console.log(`[TranslateQueue] Done (${job.reason}): ${job.articleId} ${lang}`);
          } catch (err) {
            console.error(`[TranslateQueue] Failed (${job.reason}): ${job.articleId} ${lang}:`, err.message);
          } finally {
            inFlight.delete(`${job.articleId}:${lang}`);
          }
        })
      );
      if (jobs.length > 0) await sleep(GAP_BETWEEN_LANGS_MS);
    }
  } finally {
    working = false;
  }
...
function queueTranslation(articleId, { delayMs = DEBOUNCE_MS, reason = 'edit' } = {}) {
  ...
  const timer = setTimeout(() => {
    ...
    // User-triggered: jump ahead of any queued reconcile/backfill jobs.
    enqueueRun(articleId, SUPPORTED_LANGUAGES, reason, { priority: true });
  }, delayMs);
