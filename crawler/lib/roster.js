...
async function loadRoster(supabase) {
  const { data, error } = await supabase.from('client').select('id,name,status,seo_level,website,gsc_property_url').in('status', ['Live', 'Onboarding'])
  if (error) throw new Error(`roster: ${error.message}`)
  return filterRoster(data)
}

module.exports = { NON_CONTENT_SEO_LEVELS, TEST_ROW_NAME, EXCLUDED_CONTENT_CLIENT_IDS, filterRoster, loadRoster }
