...
function mapGscResponse(payload, sitePages)  {
  ...
  for (const row of payload?.rows || [])  {
    let path;
    try  {
      path = normalizePath(new URL(row.keys?.[0]).pathname)
    }
    catch  {
      continue
    };
    ...
    if (page) result.push( {
      url: page.url, clicks_30d: Math.round(Number(row.clicks) || 0), impressions_30d: Math.round(Number(row.impressions) || 0)
    })
  }
  return result
}
async function getAccessToken(credentials, fetchImpl = global.fetch)  {
  ...
  if (!payload.access_token) throw new Error('gsc token missing access_token');
  return payload.access_token
}
function queryDates(now = new Date())  {
...
async function fetchGsc(propertyUrl, sitePages, accessToken, fetchImpl = global.fetch, now = new Date())  {
  const response = await fetchImpl(`https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(propertyUrl)}/searchAnalytics/query`,  {
    method: 'POST', headers:  {
      Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json'
    }, body: JSON.stringify( {
      ...queryDates(now), dimensions: ['page'], rowLimit: 5000
    })
  })
  if (response.status === 403) return  {
    rows: [], noAccess: true
  };
  if (!response.ok) throw new Error(`gsc HTTP ${response.status}`);
  return  {
    rows: mapGscResponse(await response.json(), sitePages), noAccess: false
  }
}
module.exports =  {
  mapGscResponse, getAccessToken, queryDates, fetchGsc
}
